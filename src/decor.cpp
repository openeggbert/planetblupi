...
// On ne peut pas aller dans la derničre cellule tout au
...
CDecor::~CDecor()
{
	UndoClose();  // libčre le buffer du undo
}
...
// Initialise le décor aprčs une modification.
...
void CDecor::ClearFire()
{
	...
	for ( x=0 ; x<MAXCELX/2 ; x++ )
	{
		for ( y=0 ; y<MAXCELY/2 ; y++ )
		{
			if ( m_decor[x][y].fire >= MoveMaxFire() )  // déjŕ brulé ?
			{
...
void CDecor::BuildPutBlupi()
{
...
#if 0  // déjŕ fait au point (1), voir Build
	for ( x=0 ; x<MAXCELX ; x++ )
	{
		for ( y=0 ; y<MAXCELY ; y++ )
		{
	...
	for ( rank=0 ; rank<MAXBLUPI ; rank++ )
	{
			 ...
			 m_blupi[rank].icon    != -1 )
		{
			...
			if ( m_rankBlupi[x][y] != -1 )  // déjŕ occupé ?
			{
...
void BuildMoveTransport(int icon, POINT &pos)
{
	...
	if ( icon == 106 )  // se penche (mčche dynamite) ?
	{
		pos.x += 8;
		pos.y += 10;
	}

	if ( icon == 194 )  // se penche (mčche dynamite) ?
	{
		pos.x += 9;
		pos.y += 9;
	}
...
//				 m_decor[x/2][y/2].floorIcon    >= 0 )
			if ( x%2 == 0 && y%2 == 0 )
			{
				...
				if ( !m_bBuild && icon == 71 )  // terre ŕ fer ?
				{
...
void CDecor::Build(RECT clip, POINT posMouse)
{
		...
		for ( i=0 ; i<nbx ; i++ )
		{
				 ...
				 y >= 2 && y < MAXCELY-2 )
			{
					 ...
					 !m_blupi[rank].bCache )
				{
					...
					// Dessine la flčche ronde "répčte" sous blupi.
					...
					// Dessine la flčche jaune sur blupi.
	...
	if ( m_shiftOffset.x < 0 )  // décalage ŕ droite ?
	{
		nbx += 2;
	}
	...
	if ( m_shiftOffset.x > 0 )  // décalage ŕ gauche ?
	{
		nbx += 2;
		iCel.x --;
		iCel.y ++;
	...
	term:
	// Dessine la flčche jaune sur un objet.
	if ( m_celArrow.x != -1 )
	{
...
// Augmente la phase.
//	-1	mise ŕ jour continue
//	 0	début de mise ŕ jour périodique
//	 1	mise ŕ jour périodique suivante
	...
	if ( action == WM_ACTION_GO )
	{
		...
		if ( m_nbBlupiHili > 0 )
		{
			...
			for ( i=0 ; i<nb ; i++ )
			{
				...
				{
//?					icons[1+x][1+y] = ICON_HILI_GO;  // flčche
		 ...
		 action <= WM_ACTION_BUILD6 )
	{
		...
		else
		{
			...
			{
				GetFloor(cel, channel, icon);
				if ( channel != CHFLOOR || icon != 71 )  // terre ŕ fer ?
				{
					error = ERROR_GROUND;  // sol pas adéquat
				}
			}
			...
			if ( action == WM_ACTION_BUILD6 &&   // téléporteur ?
				 CountFloor(CHFLOOR, 80) >= 2 )  // déjŕ 2 ?
			{
				error = ERROR_TELE2;  // déjŕ 2 téléporteurs
			}
...
// Retourne le rang du ničme blupi sélectionné.
...
// Retourne l'identificateur du texte correspondant ŕ
...
// Retourne le pointeur ŕ la liste des boutons existants.
...
// Revient en arričre pour tout le décor.
