...
{
	short		bExist;
	short		perso;		// -1=objet, -2=feu, -3=flčche
...
// Retourne la statistique correspondant ŕ un rang donné.
...
// Met ŕ jour tous les compteurs des statistiques.
...
void CDecor::StatisticDraw()
{
	...
	for ( rank=0 ; rank<STATNB ; rank++ )
	{
		...
		if ( rank == 0 && m_bStatUp )
		{
			...
			m_pPixmap->DrawIcon(-1, CHBUTTON, icon, pos);  // flčche up
			continue;
		}

		if ( rank == STATNB-1 && m_bStatDown )
		{
			icon = 6+68;  // flčche down
			if ( rank == m_statHili )  // statistique survolée ?
			{
				icon ++;
			}
			pos.x += 23;
			pos.y -= 5;
			m_pPixmap->DrawIcon(-1, CHBUTTON, icon, pos);  // flčche down
			continue;
		}
...
void CDecor::GenerateStatictic()
{
	...
	{
		StatisticUpdate();  // met ŕ jour les compteurs
	}
...
BOOL CDecor::StatisticDown(POINT pos, int fwKeys)
{
	...
	if ( m_bStatDown && hili == STATNB-1 )  // flčche down ?
	{
		if ( m_statFirst == 0 )
		{
