...
// une fois le but trouvé, reprend en arričre
// ŕ la recherche du chemin le plus court

// retourne la direction ŕ prendre
...
// troisičme méthode de recherche
// semblable ŕ la précédente,
// mais les points ŕ explorer sont classés selon leur distance ŕ la cible
...
void CDecor::CheminFillTerrain(int rank)
{
	...
	m_cheminWork[pos] = 1;  			// premičre position

	// répčte jusqu'ŕ trouvé ou plus de possibilités
	max = 500;
	while ( max-- > 0 )
	{
...
// retourne l'incrément pour passer ŕ la nouvelle case
// et le "prix ŕ payer" pour aller dans cette direction
// coűt doit ętre déterminé en sortie
								 ...
								 int &cout, int &action)
{
	...
	cout *= ampli; 					// coűt plus élevé si grande amplitude
...
//?				if ( m_blupi[m_blupiHere].goalCel.x == -1 )  return FALSE;
				...
				// Si blupi mobile, possible mais coűt élevé.
				cout = 20;
			}
		}
...
// Retourne TRUE si on a assigné une nouvelle direction ŕ blupi.
BOOL CDecor::CheminCherche(int rank, int &action)
{
    ...
    // cherche le chemin ŕ partir de la destination
...
// Teste s'il est possible de se rendre ŕ un endroit donné.
