	...
	for ( rank=0 ; rank<MAXBLUPI ; rank++ )
	{
			 ...
			 (perso == -1 || m_blupi[rank].perso == perso) )
		{
			...
			if ( !m_bBuild )  // phase de jeu ?
			{
				...
				{
					MoveFinish(rank);  // stoppe décompte ŕ rebourd
				}
				if ( m_nbBlupiHili > 0 &&
...
// Supprime tout dans un périmčtre donné suite ŕ une explosion.
//	type=0	->	explosion
//	type=1	->	électro
...
void CDecor::BlupiKill(int exRank, POINT cel, int type)
{
	...
	for ( rank=0 ; rank<MAXBLUPI ; rank++ )
	{
		...
		// dans la cellule oů a lieu l'explosion.
		if ( rank != exRank &&
			 m_blupi[rank].bExist &&
			 ...
			 m_blupi[rank].cel.y <= cel.y+1 )
		{
			...
			if ( type == 1 )  // électro ?
			{
				...
				if ( m_blupi[rank].perso == 0 &&
					 m_blupi[rank].vehicule == 0 &&  // ŕ pied ?
					 !m_bInvincible &&
					 icon != 113 &&                 // maison ?
...
//?					BlupiChangeAction(rank, ACTION_ELECTRO);
		...
		// cellule oů a lieu l'explosion, seulement s'ils
...
void CDecor::BlupiInitAction(int rank, int action, int direct)
{
	...
	if ( m_blupi[rank].perso == 0 )  // blupi ?
	{
		...
		if ( m_blupi[rank].action == ACTION_STOP &&
			 m_blupi[rank].goalAction == 0 )  // ŕ pied ?
		{
			rand = Random(0,400);
			if ( rand >= 10 && rand <= 15 &&
				 m_blupi[rank].takeChannel == -1 )
...
// Retourne le paramčtre associé ŕ une action.
...
BOOL CDecor::ListPut(int rank, int button, POINT cel, POINT cMem)
{
	...
	// Mémorise "mange" seulement aprčs un "cultive".
	if ( button == BUTTON_MANGE &&
		 m_blupi[rank].listButton[0] != BUTTON_CULTIVE )  return TRUE;

	// Si prend/dépose ŕ la suite au męme endroit,
...
// Enlčve la derničre action ajoutée dans la liste.
...
// Cherche une action ŕ répéter dans la liste.
...
//?	if ( button == BUTTON_MANGE )
//?	{
//?		cel = m_blupi[rank].cel;  // cherche lŕ oů est blupi !
//?	}
...
// Permet de passer ŕ travers certains arbres.
...
//?		m_blupi[rank].passCel.x = -1;
	...
	if ( op == GOAL_LABO )
	{
		...
		{
			m_blupi[rank].takeIcon = 93;  // pičge
		}
	...
	if ( op == GOAL_NEWBLUPI )
	{
		cel.x = m_blupi[rank].cel.x+(*pTable++);
		cel.y = m_blupi[rank].cel.y+(*pTable++);
		PutObject(cel, -1,-1);  // enlčve les oeufs
	...
	if ( op == GOAL_USINEFREE )
	{
		...
		{
			GoalStop(rank, TRUE);
			m_blupi[rank].goalCel = GetCel(cel,1,-1);  // ŕ côté de la porte
//?			m_blupi[rank].goalAction = 0;  // stoppe sitôt aprčs
//?			m_blupi[rank].interrupt = 1;
//?			GoalUnwork(rank);
//?			FlushUsed(rank);
		}
	...
	if ( op == GOAL_VEHICULE )
	{
		...
		if ( m_blupi[rank].vehicule != 0         &&  // pas ŕ pied ?
...
// Stoppe complčtement une action.
...
void CDecor::GoalStop(int rank, BOOL bError, BOOL bSound)
{
	...
	if ( bError && bSound )
	{
		ListRemove(rank);  // supprime la derničre action mémorisée
	}
...
// Teste si une cellule est déjŕ utilisée comme but pour
...
BOOL CDecor::BlupiNextAction(int rank)
{
	...
	// Si blupi est presque complčtement épuisé, il stoppe.
	if ( !bOK &&
		 m_blupi[rank].perso == 0 &&
...
void CDecor::BlupiNextGoal(int rank)
{
	...
	// Si blupi passe trop pręt du feu, aie aie aie ...
...
//?			goto init;
	...
	// Si blupi passe trop pręt d'un virus ...
	if ( m_blupi[rank].perso == 0 &&
	...
	// Si blupi est complčtement épuisé, il meurt.
	if ( m_blupi[rank].perso == 0 &&
		 m_blupi[rank].energy == 0 )
	{
		BlupiDeselect(rank);
...
//			}
		 ...
		 m_blupi[rank].goalAction != WM_ACTION_T_DYNAMITE )
	{
		...
		if ( IsBlupiHere(cel, FALSE) &&
			 ...
			 m_blupi[m_blupiHere].vehicule == 0 )  // ŕ pied ?
		{
			m_blupi[rank].bExist = TRUE;
			// Blupi écrasé au sol.
...
//-					}
				...
				else
				{
					...
					{
						if ( m_blupi[rank].busyCount == 0 )  // derničre tentative ?
						{
							GoalStop(rank, TRUE);
							m_blupi[rank].goalCel.x = -1;
							m_blupi[rank].goalPhase = 0;
...
// Met ou enlčve une flčche au blupi sélectionné blupi.
	...
	if ( m_bHiliRect )  // rectangle de sélection existe ?
	{
		...
		if ( nb > 0 )
		{
			...
			else
			{
				...
				if ( bEnerve )  // déjŕ sélectionné y'a peu ?
				{
...
void CDecor::BlupiDrawHili()
{
	...
	cc.x = c1.x;
	cc.y = c2.y;
	p3 = ConvCelToPos(cc);  // p3 ŕ gauche

	cc.x = c2.x;
	cc.y = c1.y;
	p4 = ConvCelToPos(cc);  // p4 ŕ droite
...
// Retourne le bouton par défaut ŕ un endroit donné.
...
int CDecor::GetDefButton(POINT cel)
{
	...
	if ( channel == CHOBJECT )
	{
		...
		if ( iCel.x%2 == 1 && iCel.y%2 == 1 )
		{
			...
			if ( icon ==  93 )  button = BUTTON_CARRY;   // pičge
...
// Indique un but visé ŕ long terme, pour un blupi donné.
...
// Indique un but visé ŕ long terme, pour tous les blupi
// sélectionnés.
...
// contient un blupi ŕ pied ou un détonnateur de mine
...
// Retourne TRUE si un blupi est déjŕ sélectionné et qu'il
...
// Retourne les boutons possibles ŕ un endroit donné,
							 ...
							 int *pButtons, int *pErrors, int &perso)
{
	...
	if ( m_rankBlupiHili >= 0 )
	{
		if ( m_blupi[m_rankBlupiHili].energy > MAXENERGY/4 &&
			 ...
			 m_blupi[m_rankBlupiHili].vehicule == 0 )  // ŕ pied ?
		{
			bBuild = TRUE;
		}
	}
	...
	while ( table_buttons[i] != -1 )
	{
		...
		if ( bBuild &&
			 table_buttons[i+1] != 0 &&  // toujours présent si matičre ?
			 (m_rankBlupiHili < 0 ||
			  ...
			  table_buttons[i+1] != 44) )  // ni pierres ?
		{
			...
			if ( channel == CHOBJECT &&
				 icon == table_buttons[i+1] &&  // matičre ?
				 cel.x%2 == 1 && cel.y%2 == 1 )
			{
	...
	// Regarde s'il faut ajouter le bouton "répčte".
	...
	for ( i=0 ; i<nb ; i++ )
	{
		...
		if ( rank > 0 )  // au moins 2 actions ŕ répéter ?
		{
