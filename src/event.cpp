...
typedef struct
{
	...
	short			speed;
	short			bMovie;
	short			maxMission;		// derničre mission effectuée (0..n)
	short			scrollSpeed;
	short			audioVolume;
	short			midiVolume;
	short			bAccessBuild;
...
// Toutes les premičres lettres doivent
// ętre différentes !
...
// Restitue le jeu aprčs une activation en mode fullScreen.
...
// Crée tous les boutons nécessaires ŕ la phase en cours.
...
// Retourne le lutin ŕ utiliser ŕ une position donnée.
...
// Met ou enlčve le sablier de la souris.
...
BOOL CEvent::EventButtons(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	// Cherche le tool tips ŕ utiliser pour la souris.
	m_textToolTips[0] = 0;
...
BOOL CEvent::ChangePhase(UINT phase)
{
	...
	m_pDecor->UndoClose();  // libčre le buffer undo
	...
	{
		WaitMouse(FALSE);  // enlčve le sablier
		m_tryInsertCount = 40;
		m_tryPhase = m_phase;
	...
	m_pDecor->HideTooltips(FALSE);
	m_menu.Delete();
	m_pDecor->BlupiSetArrow(0, FALSE);  // enlčve toutes les flčches
	m_pDecor->ResetHili();  // enlčve les mises en évidence
...
{
	if ( m_movieToStart[0] != 0 )  // y a-t-il un film ŕ démarrer ?
	{
...
//?		  m_phase == WM_PHASE_BUILD) )
		 ...
		 m_phase == WM_PHASE_BUILD )
	{
		...
		if ( m_shiftPhase > 0 )
		{
			...
			if ( m_shiftPhase == 0 )  // derničre phase ?
			{
				offset.x = 0;
				offset.y = 0;
				m_pDecor->SetShiftOffset(offset);
...
void CEvent::BuildFloor(POINT cel, int insIcon)
{
	...
	if ( insIcon ==  99 ||  // station de recharge ?
		 ...
		 (insIcon >= 106 && insIcon <= 112) ||  // barričre ?
...
// Enlčve si nécessaire un objet sur l'eau.

void CEvent::BuildWater(POINT cel, int insIcon)
{
	int		channel, icon;

	if ( insIcon != 14 )  return;  // rien ŕ faire si pas eau
	...
	if ( channel == CHOBJECT &&
		 ...
		 icon != 117 )   // pas bateau ?
	{
		m_pDecor->PutObject(cel, -1, -1);  // enlčve l'objet
		m_pDecor->ArrangeObject(cel);
	}
}

// Cette table donne les objets ŕ construire en fonction
...
static int tableHome[] =
{
	...
	112,0,0,0,0,0,0,0,0,0,				//  5 barričre
	26,71,0,0,0,0,0,0,0,0,				//  6 palissade
	37,38,39,40,41,42,43,0,0,0,			//  7 rochers
	36,44,60,63,80,123,14,0,0,0,		//  8 matičres
	85,125,93,92,0,0,0,0,0,0,			//  9 pičges
...
BOOL CEvent::BuildDown(POINT pos, int fwKeys, BOOL bMix)
{
	...
	if ( GetState(WM_DECOR1) == 1 )  // pose d'un sol
	{
		...
		else
		{
			...
			{
				BuildWater(cel, icon);  // enlčve les objets
			}
...
BOOL CEvent::StartMovie(char *pFilename)
{
	...
	rect.left   = 1;  // mystčre: plante avec 0,0,LXIMAGE,LYIMAGE !!!
	rect.top    = 1;
...
// Retourne la derničre position de la souris.
...
BOOL CEvent::TreatEventBase(UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch( message )
    {
		case WM_KEYDOWN:
			...
			{
				if ( m_posCheat == 0 )  // premičre lettre ?
				{
					m_rankCheat = -1;
					for ( i=0 ; i<9 ; i++ )
					{
...
//?			DecorAutoShift(pos);
		...
		case WM_PHASE_MISSION:
			m_bSchool  = FALSE;
			m_bPrivate = FALSE;
			if ( m_mission == 0 )  // premičre mission ?
			{
		...
		case WM_PHASE_UNDO:
			m_pDecor->UndoBack();  // revient en arričre
			break;
...
// (*)	Sans cela, il existe un drôle de problčme sur
//		certains PC: la souris ne suis pas le mouvement
//		réel effectué.
//		Problčme résolu par Denis !


// Affiche un caractčre de debug tout en haut.
