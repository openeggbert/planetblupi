...
// l'eau lorsque la valeur est ŕ un.
//		0  1
//		2  3
...
// Cette table indique les quarts de cases contenant de
// la mousse ou de la terre lorsque la valeur est ŕ un.
//		0  1
//		2  3
...
// Arrange le sol aprčs une modification.
...
void CDecor::ArrangeFloor(POINT cel)
{
	...
	for ( index=0 ; index<max ; index++ )
	{
		...
		test.x = cel.x -2;		// en bas ŕ gauche
		test.y = cel.y +2;
		...
		test.x = cel.x -2;		// en haut ŕ gauche
		test.y = cel.y -2;
		if ( GetSeeBits(test, bits, index) )
		{
			if ( bits[0] == here[0] &&
		...
		test.x = cel.x +2;		// en haut ŕ droite
		test.y = cel.y -2;
		if ( GetSeeBits(test, bits, index) )
		{
			if ( bits[1] == here[1] &&
		...
		test.x = cel.x +2;		// en bas ŕ droite
		test.y = cel.y +2;
		if ( GetSeeBits(test, bits, index) )
		{
			if ( bits[3] == here[3] &&
...
// Arrange les objets aprčs une modification.
...
// Rempli un sol ŕ partir d'une position donnée.
