/**
 * @file resource.h
 * @brief Win32 resource identifiers generated by MSVC for Planet Blupi.
 *
 * Contains all TX_* string resource IDs used throughout the engine for:
 *  - Toolbar action tooltips (TX_ACTION_*)
 *  - UI button labels (TX_BUTTON_*)
 *  - Win/lose screen messages (TX_WIN*, TX_LOST*, TX_LASTWIN*)
 *  - Object names shown in the statistics panel (TX_OBJ_*)
 *  - Error messages (TX_ERROR_*)
 *  - Direction labels (TX_DIRECT_*)
 *  - In-game embedded WAV resources (IDR_WAVE_BOING, IDR_WAVE_BLOW)
 *
 * These IDs are passed to LoadString() / misc.h to retrieve the
 * localised text for the active language.
 */
