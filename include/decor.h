...
typedef struct
{
	...
	POINT	pos;			// position relative ŕ partir de la cellule
	...
	int		nbUsed;			// nb de points déjŕ visités
	char	nextRankUsed;
	...
	short	bArrow;			// TRUE -> flčche en dessus de blupi
	short	bRepeat;		// TRUE -> répčte l'action
...
protected:
	...
	POINT		m_celArrow;			// cellule avec flčche
	BOOL		m_bOutline;
	BOOL		m_bGroundRedraw;
	char		m_buttonExist[MAXBUTTON];
	int			m_statNb;			// nb de statistiques
	int			m_statFirst;		// premičre statistique visible
	int			m_bStatUp;			// flčche up statistique
	int			m_bStatDown;		// flčche down statistique
	...
	int			m_lastRegion;		// numéro derničre région
	int			m_blupiHere;
